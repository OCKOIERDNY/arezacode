  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        if (process.env.AREZACODE_AUTOMATIONS !== "0") hooks.push(yield* Effect.promise(() => AutomationsPlugin(input)))
        ...
        for (const load of loaded) {
          if (!load) continue
          if (process.env.AREZACODE_AUTOMATIONS !== "0" && load.mod.EntirePlugin) {
            const source = yield* Effect.promise(() =>
              readFile(load.entry.startsWith("file:") ? new URL(load.entry) : load.entry, "utf8").catch(() => ""),
            )
            if (
              source.startsWith(
