...
test("scrolls a narrow file panel horizontally without expanding the sidebar", async ({ page }) => {
  ...
  await expect.poll(() => body.evaluate((el) => el.scrollLeft)).toBeGreaterThan(300)
  ...
  await page.mouse.down()
  await page.mouse.move(bounds.x + bounds.width - 30, bounds.y + bounds.height - 5, { steps: 10 })
  await page.mouse.up()
  await expect.poll(() => body.evaluate((el) => el.scrollLeft)).toBeGreaterThan(200)
  await expect(tree).toHaveCSS("width", "480px")
})
