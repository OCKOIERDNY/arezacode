...
export const SessionQuestionDock: Component<{ request: QuestionRequest; onSubmit: () => void }> = (props) => {
  ...
  const count = createMemo(() => options().length + Number(customEnabled()))
  ...
  const pickFocus = (tab: number = store.tab) => {
    ...
    if (questions()[tab]?.custom !== false && store.customOn[tab]) return list.length
    return Math.max(
      0,
  ...
  const submit = () =>
    void reply(questions().map((item, i) => {
      const answers = store.answers[i] ?? []
      if (item.custom !== false) return answers
      return answers.filter((answer) => item.options.some((option) => option.label === answer))
    }))

  const answered = (i: number) => {
    if ((store.answers[i]?.length ?? 0) > 0) return true
    return questions()[i]?.custom !== false && store.customOn[i] && (store.custom[i] ?? "").trim().length > 0
  }
  ...
  const customToggle = () => {
    if (sending() || !customEnabled()) return
