...
- Independent prompts are queued task-context boundaries inside the existing durable Session transcript. Provider history starts at the latest promoted independent user message; visible history is retained. Implicit followups submitted behind a pending independent boundary queue behind it, and exact retries retain their recorded delivery. Switching isolation off continues the latest task context. Context limits reset only on a new promoted boundary; late background results must match their originating task context.
